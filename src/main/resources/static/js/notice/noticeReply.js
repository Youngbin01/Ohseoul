...
$(document).ready(function () {
    ...
    $("#replyWrite").submit(function (event) {
        ...
        const boardNoticeId = $("#a").val();
        // CSRF 토큰 가져오기
        const token = $("meta[name='_csrf']").attr("content");
        const header = $("meta[name='_csrf_header']").attr("content");
        // AJAX 요청 보내기
        $.ajax({
            url: "/user/noticeReply/insert/" + boardNoticeId, // boardCommunityId를 URL에 포함시킴
            type: "POST",
    ...
    $("#replyUpdate").click(function() {
        ...
        $.ajax({
            url: "/user/noticeReply/update/"+boardNoticeId,  // 현재 위치를 돌아오기위해서 사용
            type: "PUT",
            contentType: "application/json",
    ...
    $(".replyDelete").click(function() {
        ...
        $.ajax({
            url: "/user/noticeReply/delete/"+replyId,  // 삭제
            type: "DELETE",
            contentType: "application/json",
            data: JSON.stringify(requestData), // JSON 형식으로 데이터 전송
            beforeSend: function(xhr){
    ...
    $("#boardDelete").click(function() {
        if (confirm('정말로 삭제하시겠습니까?')) {
            ...
            $.ajax({
                url: '/community/admin/notice/delete/' + id,
                type: 'DELETE',
                beforeSend: function(xhr) {
                    xhr.setRequestHeader(header, token);
                },
                success: function(result) {
                    alert('게시물이 삭제되었습니다.');
                    window.location.href = '/community/notice/list';
                },
